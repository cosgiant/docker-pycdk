...
```$ aws sso login --profile cos-sso-dev```

Optional Verify SSO

```$ aws sts get-caller-identity --profile cos-sso-dev```
...
```
$ cd docker-pycdk
$ aws ecr get-login-password --region us-west-2 --profile cos-sso-dev | docker login --username AWS --password-stdin $ECR
